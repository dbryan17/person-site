...
const Venmo = {
  ...
  content: `
  This tool has two functions: 1) automatically paying my rent via venmo every month, and 2) scraping my emails for utilities charges and requesting my housemates for their share.
...
const Crosswords = {
  ...
  subtitle: "Small project to assist in crossword creation",
  buttons: [
    {
  ...
  I abandoned this project before adding a large dictionary because I found a great tool already out there: %%a{https://crosshare.org/[Crosshare]}.
  `,
};

const juliaSetThreeD = {
  title: "3D Julia Set Transformations - In Progress",
  subtitle: "An extension onto Fractal Voyager",
  buttons: [
    {
  ...
  Voyager, a user will draw a line across a parameter plane fractal. Then, the code will run and it will produce an
  ...
  folds: [
    {
      ...
      content: `
      First, Fractal Voyager must generate many dynamical plane fractals along the line. Each fractal is converted to a black and white image where black pixels are in the set. Then, this is converted to a binary that represents if each pixel is black or white, which is passed to a separate program (for now).%%n
      ...
      Once we do the above for each cross section, we have a list of triangles that represent the 3D shape. This is passed to a function which generates an stl file for all the triangles.
      `,
    },
  ],
  imgs: [
...
const MtpDb = {
 ...
 approximately ten percent of all the data on the guidebook application, corresponding to over one million rows in one of the tables. 
  ...
  folds: [
    {
      ...
      online guidebook and social media site mountain project. For example, what area has the most easy and safe climbs? Who is the biggest %%a{https://www.toprockclimbing.ca/blog/2019/5/7/why-does-sandbagging-occur#:~:text=Sandbag.,easier%20than%20the%20declared%20grade.[sandbagger]}? 
      What climbs do the OG climbers love? Of course, this would be very inaccurate since not all routes and ascents are logged on mountain project, and I was only able to gather about 
  ...
  folds: [
    {
      ...
      Another unique structure of the sudoku graph is its cliques.
      A clique is a complete subgraph, of which there a 27 with 9 vertices in the sudoku graph. Each block, row, and column is a 9-vertex-clique because no vertex in any one of those sets can be the same symbol as any other, giving us an edge between every vertex, thus a complete subgraph. 
      `,
      innerFold: {
        ...
        A valid coloring is one in which the color of each vertex is unique from the color of all vertices that it is adjacent to. 
        So, all graph cliques have a exactly 9 colors. In terms of the grid, this means that each row, column, and block has unique numbers. %%n
        In the application, two backtracking algorithms were implemented to 
        complete the graph coloring of a sudoku. It is worth noting that a valid sudoku has exactly one 9-color solution. A different backtracking algorithm to generate valid grids ensures this.
...

    {
      ...
      The solving strategies are ranked from easiest to hardest, so the easiest strategies are shown first.
...
const FractalVoyager = {
  ...
  It includes a simple scripting language based on 
  %%a{https://pi.math.cornell.edu/~noonan/fstream.html[Fractal Stream]}.
  This language, in conjunction with app features, enables users to
  ...
  folds: [
    {
      ...
      allowing scripts to be entered and compiled on the web. Then, the
    ...
    },
    {
      ...
      content: `
      The language and application supports parameter and dynamical plane fractals, 
      ...
      For example, z = z^2+c. It includes the parameter, c*, which changes with respect 
      ...
      innerFold: {
        ...
        content: `
        The compiled WebAssembly is parameterized, allowing users to
        explore a function without needed to recompile. A key
        feature of this is switching between the parameter plane and

    ...
    <>
      <section className="pageChunk">
        ...
        <Fold
          ...
          foldText={"Learn why Fractal Voyager is fast (WebAssembly)"}
        />
        <Fold
          foldContent={
            <p className="inner-page-chunk">
              The language and application supports paramter and dynamical plane
              fractals, along with point orbit visualizations. The parameter
              plane is the complex plane when a function is iterated until a
              condition is met with respect to a fixed point. For example, z =
              z^2+c. It includes the paramter, c*, which changes with respect to
              the passed in point across the complex plane.{"\n\n"}The dynamical
              plane is the complex plane when a function is iterated until a
              condition is met with respect to a variable point, z, that changes
              with respect to the passed in point across the complex plane. For
              example, z = z^2 − 0.79 + 0.15i. A complex number’s orbit for a
              particular dynamical system is the value of the number after each
        ...
        <Fold
          foldContent={
            <p className="inner-page-chunk">
              ...
              explore a function without needed to recompile. A key feature of
              this is switching between the paramter plane and dynamical plane
              by selected a point to fix the c value. One can also select a
              point in the dynamical plane to see it&apos;s orbit, all done
              without needing to write a new script. A dynamical plane fractal
              can also be generated by writing a script without a c value.{" "}
              {"\n"}
              The fractal images can be altered by changing the breakpoints of
              the funciton conditions. For example, one condition that can be
              put on a funciton is &quot;until z stops&quot;, which means
              consider a point in the fractal set if z never converges. For this
              condition, one can alter the epsilon value: the difference between
              the current value of z versus the last value of z to say the
              function converged.
            </p>
          }
